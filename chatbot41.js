...
Promise.all([
    ...
    * { margin: 0; padding: 0; box-sizing: border-box; font-family: "Inter", sans-serif; }
    ...
    const updateSendButtonVisibility = () => {
        ...
        sendMessage.style.display = (hasMessage || hasFile) ? "block" : "none";
      };
...
const resetChatHistory = async () => {
    ...
    try {
        ...
        // 1. First try with FormData
        let formData = new FormData();
        formData.append("session_id", String(currentBrowserId));
        ...
        // If first attempt fails, try with JSON
        if (!response.ok) {
            ...
            response = await fetch("https://khadargroups-ai-chatbot.vercel.app/clear", {
                ...
                body: JSON.stringify({
                    session_id: String(currentBrowserId)
                })
            });
        }
    ...
    messageInput.addEventListener("input", () => {
        ...
        sendMessage.style.display = hasContent ? "block" : "none";
      });
