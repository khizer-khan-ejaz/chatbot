  ...
  .chat-form .file-upload-wrapper.file-uploaded #file-cancel {
    ...
    if (!storedToken || storedToken !== "199908") {
      ...
      const generateBotResponse = async (incomingMessageDiv) => {
        ...
        try {
            // API call instead of Firebase query
            const apiResponse = await fetch('https://fastapi-vercel-iota.vercel.app/chat', {
                ...
                body: JSON.stringify({
                    message: userMessage,
                    browserId: currentBrowserId // Send browser ID with the request
                }),
            });
            ...
            let botResponseText = data.response || "Sorry, I don't understand that yet."; // Fallback if response is empty
    
            // Convert any URLs in the response into clickable links, avoiding trailing punctuation
            botResponseText = botResponseText.replace(
                ...
                (url) => {
                    ...
                    return `<a href="${url}" target="_blank" class="bot-link">${url}</a>`;
                }
            );
    
            // Display bot's response
            await new Promise((resolve) => setTimeout(resolve, 1500)); // Simulate typing delay
            messageElement.innerHTML = botResponseText; // Use innerHTML to render the clickable link
